...
public class Sorts {
	...
	/**
	 * Swaps two elements of an array, indicated by the indices i and j
	 * @param arr the array affected
	...
	private static <T extends Comparable<T>> int findMedianIndex(T[] arr, int i, int j, int k) {
		...
		if((val1.compareTo(val2) <= 0 && val1.compareTo(val3) >= 0)
			...
			// If val3 <= val1 <= val2 or val2 <= val1 <= val3, val1 (index i) is the median
			return i;
		} else if((val2.compareTo(val1) <= 0 && val2.compareTo(val3) >= 0)
				|| (val2.compareTo(val1) >= 0 && val2.compareTo(val3) <= 0)) {
			// Else if val3 <= val2 <= val1 or val1 <= val2 <= val3, val2 (index j) is the median
			return j;
		} else {
			// Otherwise return the last remaining element val3 (index k), since it's the median
			return k;
		}
	}
	...
	public static <T extends Comparable<T>> void quickSort(T[] arr, int low, int high,
		...
		if(high - low > 0) {
			...
			events.add(new SwapEvent<T>(low, pivotIndex));
